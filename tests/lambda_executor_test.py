...
def test_setup_workdir_create(lambda_executor, mocker):
    ...
    os_path_join_mock = mocker.patch(
        "covalent_awslambda_plugin.awslambda.os.path.join"
    )
    ...
    mocker.patch("covalent_awslambda_plugin.awslambda.DeploymentPackageBuilder", return_value=MagicMock())
...
def test_deployment_package_builder(lambda_executor, mocker):
    ...
    deployment_package_builder_mock = mocker.patch(
        "covalent_awslambda_plugin.awslambda.DeploymentPackageBuilder",
        return_value = MagicMock()
    )
    session_mock = mocker.patch(
        "covalent_awslambda_plugin.awslambda.AWSLambdaExecutor.get_session",
...
def test_deployment_package_builder_base_setup(mocker):
    ...
    os_mkdir_mock = mocker.patch("covalent_awslambda_plugin.awslambda.os.mkdir")
    install_mock = mocker.patch("covalent_awslambda_plugin.awslambda.DeploymentPackageBuilder.install")
...
def test_upload_fileobj(lambda_executor, mocker):
    ...
    lambda_executor.get_session.return_value.__enter__.return_value.client.assert_called_once_with('s3')
...
def test_upload_file(lambda_executor, mocker):
    ...
    app_log_mock = mocker.patch("covalent_awslambda_plugin.awslambda.app_log")
    mocker.patch("covalent_awslambda_plugin.awslambda.os.path.exists", return_value=True)
    mocker.patch("covalent_awslambda_plugin.awslambda.DeploymentPackageBuilder.__enter__", return_value=None)
    mocker.patch("covalent_awslambda_plugin.awslambda.DeploymentPackageBuilder.__exit__", return_value=None)
    mocker.patch("covalent_awslambda_plugin.awslambda.DeploymentPackageBuilder.install", return_value=None)
    mocker.patch("covalent_awslambda_plugin.awslambda.ZipFile")
    mocker.patch("covalent_awslambda_plugin.awslambda.open")
...
def test_upload_file_exception(lambda_executor, mocker):
    ...
    client_error_mock = botocore.exceptions.ClientError(MagicMock(), MagicMock())
    lambda_executor.get_session.return_value.__enter__.return_value.client.return_value.upload_file.side_effect = client_error_mock
...
def test_create_lambda_invocation(lambda_executor, mocker):
    ...
    lambda_executor._create_lambda.assert_called_once_with(f"lambda-{dispatch_id}-{node_id}", f"archive-{dispatch_id}-{node_id}.zip")
...
def test_create_lambda_iam_get_role_exception(lambda_executor, mocker):
    ...
    client_error_mock = botocore.exceptions.ClientError(MagicMock(), MagicMock())
    session_mock.return_value.__enter__.return_value.client.return_value.get_role.return_value = {"Role": {"Arn": "test_arn"}}
...
def test_is_lambda_active(lambda_executor, mocker):
    ...
    session_mock.return_value.__enter__.return_value.client.return_value.get_function.assert_called_with(FunctionName=lambda_function_name)
...
def test_invoke_lambda(lambda_executor, mocker):
    ...
    session_mock.return_value.__enter__.return_value.client.return_value.invoke.assert_called_with(FunctionName=lambda_function_name)
...
def test_teardown(lambda_executor, mocker):
    ...
    session_object_mock.assert_has_calls([call(lambda_executor.s3_bucket_name, f'func-{dispatch_id}-{node_id}.pkl')])
    session_object_mock.assert_has_calls([call(lambda_executor.s3_bucket_name, f'result-{dispatch_id}-{node_id}.pkl')])
    session_object_mock.assert_has_calls([call(lambda_executor.s3_bucket_name, f'archive-{dispatch_id}-{node_id}.zip')])
